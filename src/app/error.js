...
export default function Error({ error, reset }) {
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === "development") {
      logger.error("Application Error Boundary Triggered", errorData);
    }

    // send the error to the server in production mode
    ...
    <div className="flex items-center justify-center min-h-screen w-full bg-gradient-to-br from-[#F9FAFC] to-[#E5E8EB] flex-col gap-2 text-[#1F2025] relative overflow-hidden">
      ...
      <div className="flex items-center justify-center z-20 flex-col px-4 max-w-2xl">
        ...
        <p className="transition duration-300 text-sm md:text-base leading-7 text-[#424244] text-center mb-2 px-4">
          متاسفانه مشکلی در بارگذاری این صفحه پیش آمده است.
        </p>
        <p className="transition duration-300 text-xs md:text-sm leading-6 text-[#76767C] text-center mb-8 px-4">
          لطفاً دوباره تلاش کنید یا با پشتیبانی ژاکت تماس بگیرید.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          {/* Home Button */}
          <Link href="/">
            <div className="flex items-center justify-center h-[55px] w-[186px] rounded-lg bg-white border-2 border-[#FFAE11] text-base font-bold text-[#FFAE11] transition duration-300 hover:bg-[#FFF5E6] cursor-pointer shadow-md hover:shadow-lg transform hover:scale-105">
              <IoHomeOutline className="h-5 w-5 ml-2" />
              بازگشت به ژاکت
            </div>
          </Link>
        </div>
        ...
        <div className="mt-8">
          <Link
            href="/panel/tickets"
            className="text-sm text-[#6097F3] hover:text-[#4a7cd1] transition duration-300 "
          >

...
const ProductActions = ({ demoLink, isGrid = true }) => {
    return (
        <div className={`flex gap-2 ${isGrid ? "" : "justify-end"}`}>
            ...
            >
                <button
                    className={`${isGrid ? "w-full h-[42px]" : "h-[51px] w-full"} flex items-center justify-center gap-2 rounded-lg bg-[#EAEEF380] text-[13px] font-bold text-[#7E899B] cursor-pointer  hover:bg-[#878F9B] hover:text-white transition-colors`}
                >
               {!isGrid && <FaEye className="w-4 h-4" />}       
                    پیشنمایش
                </button>
            </Link>

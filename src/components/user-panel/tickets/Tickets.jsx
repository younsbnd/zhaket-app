...
const Tickets = () => {
  ...
  const status = useMemo(() => rawStatus || "open", [rawStatus]);

  // fetch tickets data
  const { data: ticketsResponse, isLoading } = useSWR(
    `${process.env.NEXT_PUBLIC_API_BASE_URL}/user/ticket?page=${currentPage}${status ? `&status=${status}` : ""}`,
    fetcher
  );
      ...
      <div>
        ...
        <div className="rounded-lg bg-white text-card-foreground shadow-[0px_25px_10px_0px_#5B5E6812] mx-0">
          <div className="p-6.5 pt-0 px-0 flex flex-1 flex-col min-h-[calc(100vh-350px)]">
            {/* tabs */}
            <TicketTabs counts={ticketsResponse?.counts} />
          ...
          <div className="flex justify-center py-4">
            <div>
              {/* pagination */}
              {ticketsResponse?.pagination?.totalPages > 1 && (
                <PaginationTickets
